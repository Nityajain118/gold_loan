        ...
        container.innerHTML = `
            ...
            <div class="card mb-3">
                ...
                ${allLoans.sort((a, b) => new Date(b.loanStartDate) - new Date(a.loanStartDate)).map(loan => {
                    const rate = loan.metalType === 'gold' ? settings.currentGoldRate : settings.currentSilverRate;
                    const d = Calculator.calcLoanDetails(loan, rate);
                    return `<tr class="kn-focusable" style="cursor:pointer;" onclick="LoanDetailPage.setBackTarget('customer-ledger','${customer.id}','Back to ${customer.name}'); UI.navigateTo('loan-detail','${loan.id}')">
                        ...
                        <td><button class="btn btn-outline btn-xs" onclick="event.stopPropagation(); LoanDetailPage.setBackTarget('customer-ledger','${customer.id}','Back to ${customer.name}'); UI.navigateTo('loan-detail','${loan.id}')">View</button></td>
                    </tr>`;
                }).join('')}
                </tbody></table></div>`}
            </div>
